...
if (!event) throw new Error(`Fixture event not found: ${fixtures[fixtureName]}`);

const compactEvent = {
  m: event.month_day, y: event.year, k: event.type, d: event.discipline,
  t: event.title, s: event.summary, l: event.lesson, r: event.source_label,
};
const config = {
...
fs.writeFileSync(path.join(ROOT, ".trmnlp.yml"), `${JSON.stringify(config, null, 2)}\n`);
console.log(JSON.stringify({ fixture: fixtureName, id: event.id }));
